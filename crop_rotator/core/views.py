...
#                self.the_element_family = self.the_element.family.family_master
    ...
    def check_family_slave_f(self):
        if self.the_element.is_family_slave and self.the_element.family_master is not None:
            ids = [self.the_element.family_master.id,self.the_element.id]
            query = CropFamily.objects.filter(id__in=ids)
            return query
        else:
            return CropFamily.objects.get(pk=self.the_element.id)

    def set_master_family(self):
        if self.the_element.family.is_family_slave and self.the_element.family.family_master is not None:
            return self.the_element.family.family_master

    def set_master_family_f(self):
        if self.the_element.is_family_slave and self.the_element.family_master is not None:
            return self.the_element.family_master
             ...
             about_crop=self.the_element):
                for crop_item in Crop.objects.filter(family=item.family):
                    self.add_common(
                     ...
                     "add_family_element", self.the_element,
                     )
        self.the_element.tags.add(element_to_add)
                ...
                if tag in tags_searched:
                    for interaction in tag.crop_relationships.all():
                        self.add_common(
                         interaction, item, pe_croptag_id, item.id,
                         self.the_element.id, self.the_element,
                         "add_tag_element", self.the_element,
                         )
        self.the_element.tags.add(element_to_add)
        ...
        for interaction in pe_croptag_id.crop_relationships.all():
            if interaction.about_crop is not None:
                ...
                self.add_common(
                 ...
                 "add_tag_crop_element", self.the_element,
                 )
            # WIP
                ...
                for crop_item in Crop.objects.filter(
                 family=interaction.about_family.id):
                    ...
                    self.add_common(
                     interaction, self.the_element, pe_croptag_id,
                     self.the_element.id, tag_type.id, interaction.about_crop,
                     "add_tag_crop_element_family", self.the_element,
                     )
                ...
                for item in query:
                    self.add_common(
                         ...
                         "add_tag_crop_element_tag", self.the_element,
                          )
                self.the_element.tags.add(element_to_add)
    ...
    # WIP  - do wywalenia/ przerobienia na from crop to family.
    def add_mass_family_to(self, *args):
        if args:
            self.the_element = args[0]
            self.the_element_family = self.check_family_slave_f()
        for interaction in CropInteraction.objects.filter(
         ...
         is_server_generated=False):
            for crop_item in Crop.objects.filter(family=self.the_element_family):
                self.add_common(
                 interaction, self.the_element, None, self.the_element.id,
                 crop_item.id, crop_item,
                 "add_family_to_family", self.the_element,
                 )
    ...
    def common_stuff_family3(self, query):
        ...
        for item in newquery:
            for interaction in item.crop_relationships.filter(
    ...
    def common_stuff_family2(self, q_family):
        for interaction in q_family.crop_relationships.all():
            for crop_item in Crop.objects.filter(family=q_family.id):
                if interaction.about_crop is not None:
                   tag_type = interaction.about_crop
                   self.add_common(
                    interaction, crop_item, None,
                    crop_item.id, tag_type.id, interaction.about_crop,
                    "add_to_family_by_crop", None,
                    )
                elif interaction.about_family is not None:
                    secondary_query = Crop.objects.filter(family=interaction.about_family.id)
                    for second_crop in secondary_query:
                        self.add_common(
                         interaction, crop_item, None,
                         crop_item.id, second_crop.id, second_crop,
                         "add_to_family_by_family", None,
                         )
                elif interaction.about_tag is not None:
                    secondary_query = Crop.objects.filter(tags=interaction.about_tag.id)
                    for second_crop in secondary_query:
                        self.add_common(
                         interaction, crop_item, None,
                         crop_item.id, second_crop.id, second_crop,
                         "add_to_family_by_tag", None,
                         )
    ...
    def add_mass_family(self, *args):
        if args:
            self.the_element = args[0]
            self.the_element_family = self.check_family_slave_f()
        for interaction in self.the_element_family.crop_relationships.all().filter(
         is_server_generated=False):
            for crop_item in Crop.objects.filter(family=item.id):
                self.add_common(
    ...
    def add_common(
         self, interaction, item, pe_croptag_id, item1, item2, about_crop, debug, trigger_crop):
        if item1 != item2:
...
class SyncCropTagDB(CropAdmin):
    ...
    def post(self, request, *args, **kwargs):
        if "populate_tag_db" in request.POST:
            ...
            query0 = CropFamily.objects.all()
            for cropfamily in query0:
                # Najpierw dowiąż zależności wychodzące od rodzin\
                # czyli family - crop i family-family.
                # To można ogarnąć jedną funkcją.
                #self.add_mass_family_to(cropfamily)
                self.add_mass_family_from(cropfamily)
                #self.add_mass_family(cropfamily)
            for crop in query:
                # W tym miejscu zrób zależność crop-family.
                # Też wystarczy jedna funkcja
